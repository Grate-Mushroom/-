...
public class WeatherService {
    ...
    private static final String api = "293a588b333748336de598c68458e33d";
    ...
    public WeatherData getCurrentWeather(double lat, double lon) throws WeatherApiException {
        try {
            String url = String.format("%s/weather?lat=%.4f&lon=%.4f&appid=%s&units=metric&lang=ru", Url, lat, lon, api);
    ...
    public ForecastData getForecast(double lat, double lon) throws WeatherApiException {
        try {
            String url = String.format("%s/forecast?lat=%.4f&lon=%.4f&appid=%s&units=metric&lang=ru&cnt=40", Url, lat, lon, api);
