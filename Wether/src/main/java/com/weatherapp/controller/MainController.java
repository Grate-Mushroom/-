...
public class MainController
{
    ...
    private void setupEventHandlers() {
        ...
        {
            if (newVal != null && newVal.length() >= 2) {Platform.runLater(() -> updateCitySuggestions(newVal));}
        });

        cityComboBox.setOnAction(event -> {
            City selectedCity = cityComboBox.getSelectionModel().getSelectedItem();
            if (selectedCity != null) {loadWeatherForCity(selectedCity);}
        });
    }
    ...
    private void loadWeatherForCity(City city) {
        ...
        {
            @Override
            protected Void call() throws Exception {updateWeatherData();saveFavoriteCity(city);return null;}
        };

        weatherTask.setOnSucceeded(event -> {logger.info("Weather loaded for city: {}", city.getDisplayName());});
    ...
    private void displayCurrentWeather(WeatherData weather) {
        ...
        if (weather.getWeather().length > 0) {
            ...
            } catch (Exception e) {logger.error("Failed to load weather icon", e);}
        }

        // Детали
        GridPane detailsGrid = createDetailsGrid(weather);

        VBox content = new VBox(10, cityLabel, new HBox(20, tempLabel, weatherIcon, detailsGrid), feelsLikeLabel);
    ...
    private GridPane createDetailsGrid(WeatherData weather) {
        ...
        // Ветер
        String windInfo = String.format("%.1f м/с %s", weather.getWind().getSpeed(), WindConverter.getWindDirection(weather.getWind().getDirection()));
    ...
    private void displayForecast(ForecastData forecast) {
        ...
        List<LocalDate> nextDays = forecastByDay.keySet().stream().sorted().filter(date -> !date.equals(LocalDate.now()))
    ...
    private HBox createDayForecastBox(LocalDate date, List<ForecastData.ForecastItem> forecasts) {
        ...
        double minTemp = forecasts.stream().mapToDouble(f -> f.getMain().getTempMin()).min().orElse(0);

        double maxTemp = forecasts.stream().mapToDouble(f -> f.getMain().getTempMax()).max().orElse(0);
    ...
    private void refreshWeather() {
        if (currentCity != null) {
            ...
            task.setOnSucceeded(event -> {logger.info("Weather refreshed for city: {}", currentCity.getDisplayName());});
